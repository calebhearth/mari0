	...
	for i = 1, #self.lasertable, 5 do
		if self.lasertable[i] == "left" then
			...
			if #rectcol > 0 then
				...
				newtable[i+3] = obj.x - newtable[i+1] + obj.width
		...
		elseif self.lasertable[i] == "right" then
			...
			if #rectcol > 0 then
				...
				newtable[i+3] = obj.x - newtable[i+1] + 1
		...
		elseif self.lasertable[i] == "up" then
			...
			if #rectcol > 0 then
				...
				newtable[i+4] = obj.y - newtable[i+2]
		...
		elseif self.lasertable[i] == "down" then
			...
			if #rectcol > 0 then
				...
				newtable[i+4] = obj.y - newtable[i+2] + 1 - obj.height+1
...
function laser:updateoutputs()
	for i = 1, #self.lasertable, 5 do
		...
		elseif self.lasertable[i] == "right" then
			for x = self.lasertable[i+1], math.floor(self.lasertable[i+1]+self.lasertable[i+3]) do
				local y = self.lasertable[i+2]
				self:checktile(x, y)
			end
		elseif self.lasertable[i] == "up" then
...
function laser:draw()	
	for i = 1, #self.lasertable, 5 do
		if self.lasertable[i] == "left" then
			love.graphics.setScissor(math.floor((self.lasertable[i+1]+self.lasertable[i+3]-xscroll)*16*scale), (self.lasertable[i+2]-1.5)*16*scale, math.max((-self.lasertable[i+3]+1)*16*scale, 0), 16*scale)
		...
		elseif self.lasertable[i] == "right" then
			love.graphics.setScissor(math.floor((self.lasertable[i+1]-xscroll-1)*16*scale), (self.lasertable[i+2]-1.5)*16*scale, math.max((self.lasertable[i+3])*16*scale, 0), 16*scale)
			for x = self.lasertable[i+1], math.ceil(self.lasertable[i+1]+self.lasertable[i+3]) do
				love.graphics.draw(laserimg, math.floor((x-xscroll-1)*16*scale), (self.lasertable[i+2]-20/16)*16*scale, 0, scale, scale)
			end
		elseif self.lasertable[i] == "up" then
